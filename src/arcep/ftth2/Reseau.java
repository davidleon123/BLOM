...
public class Reseau {
    ...
    public Reseau(String id, String zone, Parametres param){
        this.id = id;
    ...
    public void addPC(double x, double y, String zone, int nbLignes, boolean isPMint){
        ...
        if (distGC < param.toleranceNoeud){
            ...
            if (distGC < param.seuilToleranceGC2 && (Double) gc[0] < (Double) routier[0]){
                ...
                if(distRoutier < param.toleranceNoeud) noeud = (NoeudAGarder) n;
                else{
    ...
    public void store(File dossier){
        try{
            ...
            for (NoeudAGarder n : graphe.vertexSet()){
                if (n.hasDemandeLocale()) n.addPath(new DijkstraShortestPath(graphe, centre, n), centre);;
                n.print(writer);
            }
            writer.close();
        } catch(Exception e){
    ...
    private boolean voisin(int indice){
        switch(indice){
            case 0:
            case 2:
                return false;
        ...
        try{
            ...
            try{
                ...
                for (int i = 0; i <= iMax;i++){
                    ...
                    if (n != null && n instanceof NoeudAGarder && n.distance(coord) < param.toleranceNoeud){
    ...
    private void addArete(NoeudAGarder n1, NoeudAGarder n2, int modePose, double longueur, long idProprio, List<double[]> intermediaires, String type, boolean voisin){
        try{
            double longueurPonderee = longueur*param.multiple(modePose);
    ...
    private void addArete(NoeudAGarder n1, NoeudAGarder n2, int modePose, double longueur, long idProprio, List<NoeudInterne> intermediaires){
        ...
        graphe.addEdge(n1, n2, a);
        try{
            graphe.setEdgeWeight(a, longueur*param.multiple(modePose));
        } catch(Exception e){
            e.printStackTrace();
        }
    }
    ...
    private double distanceDijkstra(NoeudAGarder noeud){
        if (!noeud.pathConnu){
            DijkstraShortestPath dsp = new DijkstraShortestPath(graphe, centre, noeud);
            noeud.addPath(dsp, centre);
        }
    ...
    private double distanceDijkstra(Node n){
        if (n instanceof NoeudAGarder){
            ...
            try{
            double d0 = ni.distance*param.multiple(ni.a.modePose) + distanceDijkstra(ni.voisins[0]);
            double d1 = (graphe.getEdgeWeight(ni.a) - ni.distance)*param.multiple(ni.a.modePose) + distanceDijkstra(ni.voisins[1]);
            if (d0 <= d1) return d0;
            else return d1;
            } catch(Exception e){
                e.printStackTrace();
    ...
    private Object[] distanceAuCentre(double[] coord, String type){
        ...
        try{
            ...
            double d1 = n1.distance(coord)*param.multiple(11)+ distanceDijkstra(n1);
            result[0] = d1;
            result[1] = n1;
            result[2] = indice(type,false);
            if (index(indice(type,true)).size() > 0){
                Node n2 = index(indice(type,true)).nearest(coord);
                double d2 = n2.distance(coord)*param.multiple(11)+ distanceDijkstra(n2);
                if (d2<d1){
                    result[0] = d2;
                    result[1] = n2;
